...
### Hatohol DBの初期化
以下のコマンドでHatohol DBの作成と初期データの投入を行います。
...
- hatohol.confを編集することにより，作成されるDBの名前，ユーザー名，パスワードの値を変更することができます。hatohol.confは/etc/hatohol/hatohol.confに配置されています。hatohol.confの編集方法については以下をご覧ください。
